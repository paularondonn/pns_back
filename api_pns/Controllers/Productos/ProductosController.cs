            ...
            using (SqlConnection connection = conn.ConnectBD(_configuration))
            {
                ...
                try
                {
                    ...
                    while (await sqldr.ReadAsync())
                    {
                        ...
                        if (sqldr["id_product"] != DBNull.Value) { detailProducts2.idProduct = Convert.ToInt32(sqldr["id_city"]); } else { detailProducts2.idProduct = 0; }
        ...
        public async Task<IActionResult> consultProducts([FromRoute] int idProduct)
        {
            using (SqlConnection connection = conn.ConnectBD(_configuration))
            {
                ...
                try
                {
                    ...
                    while (await sqldr.ReadAsync())
                    {
                        if (sqldr["id_city"] != DBNull.Value) { detailProducts.idProduct = Convert.ToInt32(sqldr["id_city"]); } else { detailProducts.idProduct = 0; }
